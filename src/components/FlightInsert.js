...
const FlightInsert=({onInsert})=> {
    ...
    const getAirlineList = async () => {
      ...
      const onChange1=useCallback(e=>{setValue1(e.target.value);console.log(e.target.value);} ,[value1]);
     
      const onChange2=useCallback(e=>{
        airlineList.map((item)=>{
          console.log(item.airlineNm);
          if(item.airlineNm===e.target.value){setValue2(item.airlineId);
        }       
      });
      },[value2]);

      const onChange3=useCallback(e=>{
        airportList.map((item)=>{if(item.airportNm===e.target.value){setValue3(item.airportId);}});
      },[]);

      const onChange4=useCallback(e=>{
        airportList.map((item)=>{if(item.airportNm===e.target.value){setValue4(item.airportId);}});
      },[]);
      ...
      const onSubmit=useCallback(
        e=>{
          ...
          airlineList.map((item)=>{
            if(item.airlineNm===document.querySelector("#input-airline").value){
              test1=item.airlineId;
            }       
          });
          airportList.map((item)=>{
            if(item.airportNm===document.querySelector("#input-depAirport").value){
              test2=item.airportId;
            }       
          });
          airportList.map((item)=>{
            if(item.airportNm===document.querySelector("#input-arrAirport").value){
        ...
        {isLoading || isLoading2 ? (
          ...
          <form id="serchFlightForm" onSubmit={onSubmit}>
            ...
            <div className="formItem">
                ...
                placeholder="20220524 형식" onChange={onChange1}
                ></input>
            </div>
            <div className="formItem">
                <label>  항공사ㅤ: </label> 
                <select id="input-airline" className="checkedserchFlight" onChange={onChange2}>
